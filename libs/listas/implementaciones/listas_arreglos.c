...
bool InsertarOrdenadoDI(Lista lista, TipoElemento elemento){
    ...
    int pos = 0;
    while (pos < lista->cantidad && lista->valores[pos]->clave < elemento->clave) {
        pos++;
    }

    // Verificar si la clave ya existe
    if (pos < lista->cantidad && lista->valores[pos]->clave == elemento->clave) {
        printf("\nEl elemento ya estaba presente en la lista\n");
        return false;
    }

    // Desplazar elementos para insertar en pos
