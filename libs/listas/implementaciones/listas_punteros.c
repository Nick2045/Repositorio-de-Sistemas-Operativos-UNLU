...
bool InsertarOrdenadoDI(Lista lista, TipoElemento elemento){
    ...
    // Si la lista está vacía, lo inserto al inicio
    if (lista->inicio == NULL) {
        lista->inicio = nuevo_nodo;
    } else {
        struct Nodo *actual = lista->inicio;
        ...
        // Me adelanto mientras la clave actual sea menor a la del nuevo elemento
        while (actual != NULL && actual->datos->clave < elemento->clave) {
            anterior = actual;
            actual = actual->siguiente;
        }

        // Si ya existe un nodo con la misma clave, no inserto
        if (actual != NULL && actual->datos->clave == elemento->clave) {
            printf("\nEl elemento ya estaba presente en la lista\n");
            free(nuevo_nodo);  // Libero memoria
            return false;
        }
